...
import android.content.ClipboardManager // 用于 ClipboardManager
import android.content.Context // 用于 Context
...
fun Main_Page(data: Data_Manager = viewModel())
{
	...
	)
	{
		...
		)
		{
			...
			ElevatedButton(
				onClick = {
					val textToCopy = data.text // 获取要复制的文本存入变量中
					if (!textToCopy.isNullOrBlank()) // 检查文本是否不为空或不全是空格
					{
						val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager // 获取剪贴板服务
						val clip = ClipData.newPlainText("KFC文案", textToCopy)  // 创建新的剪贴板数据 一个ClipData(剪贴板内容的容器类)对象
